...
- **Global scope**: `/home/brunhilde/OpenHub/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/brunhilde/OpenHub/.claude/notes/`)
...
2. Glob `/home/brunhilde/OpenHub/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/brunhilde/OpenHub/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/brunhilde/OpenHub/.claude/notes/` — works in any directory
